...
export default function QuotationDrawer({ isOpen, quotationId, mode, onClose, onSaved }: QuotationDrawerProps) {
    ...
    <div className="fixed inset-0 z-50">
      ...
      <div className="absolute right-0 top-0 h-full w-full max-w-xl bg-white shadow-xl p-6 overflow-auto">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-gray-900">Quotation {mode === 'view' ? 'Details' : 'Edit'}</h2>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            <input name="quote_number" value={form.quote_number || ''} onChange={handleChange} disabled={mode==='view'} className="w-full px-3 py-2 border rounded-lg" />
            <input name="quote_date" type="date" value={form.quote_date ? form.quote_date.substring(0,10) : ''} onChange={handleChange} disabled={mode==='view'} className="w-full px-3 py-2 border rounded-lg" />
            <input name="valid_until" type="date" value={form.valid_until ? form.valid_until.substring(0,10) : ''} onChange={handleChange} disabled={mode==='view'} className="w-full px-3 py-2 border rounded-lg" />
